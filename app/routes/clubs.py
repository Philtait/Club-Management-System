...
from app.models import (
    Club, ClubLeader, ClubGallery,
    Membership, Admin, User, Student
)
from app.forms import (
    CreateClubForm, MembershipForm,
    ClubGalleryForm, AssignLeaderForm
)
from app.utils.notifications import send_notification, send_email
...
@login_required
def list_clubs():
    clubs = Club.query.filter_by(status="approved").order_by(Club.created_at.desc()).all()
    categories = sorted({c.category for c in clubs})
    return render_template("clubs/list.html", clubs=clubs, categories=categories)
...
def create_club():
    ...
    if form.validate_on_submit():
        club = Club(
            ...
            patron_admin_id=current_user.admin.admin_id
        )
        db.session.add(club)
        db.session.flush()
        ...
        if form.logo_file.data:
            ...
            folder = os.path.join(
                ...
                "static", "images", "clubs", str(club.club_id)
            )
...
def pending_clubs():
    ...
    pending = (
        Club.query
            .filter_by(patron_admin_id=None)
...
def approve_club(club_id):
    ...
    # notify requester
    if club.club_leaders:
        requester = User.query.get(club.club_leaders[0].user_id)
        title = "Club Request Approved"
        msg   = f"Your request to create '{club.name}' has been approved."
        send_notification(title, msg, "Club", club.club_id, [requester.user_id])
        send_email(
            to=requester.email,
            subject=title,
...
def upload_gallery(club_id):
    club = Club.query.get_or_404(club_id)
    is_leader = any(lead.user_id == current_user.user_id for lead in club.club_leaders)
    ...
    if form.validate_on_submit() and form.image_file.data:
        ...
        folder = os.path.join(
            current_app.root_path,
            "static", "images", "clubs", str(club_id), "gallery"
        )
        os.makedirs(folder, exist_ok=True)
        f.save(os.path.join(folder, filename))
...
def manage_leaders(club_id):
    ...
    if form.validate_on_submit():
        ...
        u = user = User.query.get(form.user_id.data)
